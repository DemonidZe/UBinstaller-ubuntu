...
;;
2004)
echo mysql-server-8 mysql-server/root_password password ${MYSQL_PASSWD} | debconf-set-selections
echo mysql-server-8 mysql-server/root_password_again password ${MYSQL_PASSWD} | debconf-set-selections
#ubuntu 20.04 x64 Release
apt -y install expect mysql-server libmysqlclient21 libmysqlclient-dev apache2 expat libexpat1-dev php-redis php-bcmath php-xml php-zip php-soap php-mbstring php php-cli php-mysql php-snmp libapache2-mod-php isc-dhcp-server build-essential softflowd arping snmp snmp-mibs-downloader nmap ipset automake libtool graphviz elinks php-curl ipcalc php-gd php-xmlrpc php-imap php-json >> /tmp/ubstg.log
a2enmod php
;;
1604)
